...
echo ($paged > 2 && $paged+$range+1 > $pages && $showitems < $pages)? "<a href='".get_pagenum_link(1)."'data-role=\"button\">最前</a>":"";   
echo ($paged > 1 && $showitems < $pages)? "<a href='".get_pagenum_link($prev)."'data-role=\"button\">上一页</a>":"";   
  
for ($i=1; $i <= $pages; $i++){   
if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems )){   
echo ($paged == $i)? "<span class='current ui-btn-active' data-role=\"button\">".$i."</span>":"<a href='".get_pagenum_link($i)."' class='inactive' data-role=\"button\">".$i."</a>";   
}   
}   
